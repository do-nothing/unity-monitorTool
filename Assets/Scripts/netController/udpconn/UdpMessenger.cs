...
namespace Microwise.Guide.NetConn
{
    public class UdpMessenger : Messenger
    {
        ...
        {
            mb.timestamp = long.Parse(GetTimeStamp());
            string json = mb.GetJson();
            byte[] data = Encoding.UTF8.GetBytes(json);
            server.SendTo(data, data.Length, SocketFlags.None, ipEndPoint);

            if (mb.strategy != "heartbeat")
            {
                Debug.Log(json);
            }
        }
        ...
        {
            string str = "{\"id\":\"monitor\",\"target\":\"server\",\"logType\":\"nolog\",\"strategy\":\"heartbeat\",\"quality\":0,\"timestamp\":1494825498577," +
                "\"contentBean\":{}}";
